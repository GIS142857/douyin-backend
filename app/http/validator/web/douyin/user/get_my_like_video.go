...
func (g GetMyLikeVideo) CheckParams(context *gin.Context) {
	//1.基本的验证规则没有通过
	...
	if extraAddBindDataContext == nil {
		response.ErrorSystem(context, "get_video_like 表单验证器json化失败", "")
	} else {
		// 验证完成，调用控制器，写具体业务逻辑
		(&web.UserController{}).GetMyLikeVideo(extraAddBindDataContext)
	}
}
